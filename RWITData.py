...
class SessionsDatabaseManager(DatabaseManager):
	...
	def addFromCsv(self, csvPath):
		...
		# Populate the day of the week from a timestamp
		def addWeekDayInDict(dict, timeKey = "startTime", dayKey = "weekDay"):
			...
			dict[dayKey] = abbreviations[dayInt]
			return True
			
		# Populate time slot from start/stop times
		...
		for csvRow in csvReader:
			...
			try:
				...
				if len(csvRow.get("attendees")) > 0:
					...
					addWeekDayInDict(centerSessionVals)
